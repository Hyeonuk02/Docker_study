...
7장에서는 도커 컴포즈 사용법을 다뤘다.

- 도커의 최대 장점 : 이식성
    
    → 컨테이너로 실행하기 위해 패키징된 애플리케이션은 어떤 환경에 애플리케이션을 배포해도 동일하게 작동한다.
...
- 조직에서 같은 애플리케이션을 버전을 달리해 서로 다른 환경에서 구동해야 하는 경우가 존재.
</aside>

```bash
# 애플리케이션 실행

docker-compose -f ./number/docker-compose.yml up -d
docker-compose -f ./todo-list/docker-compose.yml up -d
docker-compose -f ./todo-list/docker-compose.yml up -d
```

- 도커 컴포즈는 도커 리소스가 어떤 애플리케이션의 일부인지 아닌지를 판정하기 위해 프로젝트라는 개념을 사용한다.
- 컴포즈가 사용하는 프로젝트 이름의 기본값을 바꿀 수 있으므로 이 프로젝트 이름을 바꾸는 방법으로 단일 도커 호스트에 같은 애플리케이션을 여러 벌 실행시킬 수 있다.

![image.png](attachment:437adb4a-32ee-4b46-a7ee-11994d9c8b42:image.png)
...
- but 유지 보수 측면에서 바람직 X
- 컴포즈 파일의 내용은 90% 이상이 중복이니 수정 시 누락이 발생한다면 환경 차이 문제가 재발생.

  >> 오버라이드(덮어 쓰기) 파일은 이런 문제를 해결하는 기능

- 기본 파일을 수정하거나 해당 환경의 오버라이드 파일만 수정하거나 용도에 맞게.
- 오버라이드 파일에는 해당 환경에서 변경할 항목만 기술.
</aside>
...
- docker-compose-dev.yml : 개발 환경 대상의 설정. 도커 네트워크 및 서비스의 공개 포트를 정의하고 헬스 체크와 디펜던시 체크를 비활성화한다. 개발자들이 빠르게 애플리케이션을 실행하는 것을 목적으로 한다.
...
- 세 애플리케이션은 각각 다른 URL에서 독립적으로 동작한다.
- 자신만의 네트워크 안에서 해당 환경의 API 컨테이너와만 통신할 수 있다. 따라서 독립적.
- **도커 컴포즈는 클라이언트에서 동작하는 도구**
...
- todo-db-connection에서 비밀값을 읽어 오도록 설정 되어 있음.
- 이 값을 읽으려면 이 이름으로 정의된 비밀값이 해당 컴포즈 파일에 정의돼 있어야 한다.
- 

<aside>

- 비밀값은 도커 컴포즈, 도커 스웜, 쿠버네티스에서 모두 지원하는 기능으로, 설정값을 주입하기에 매우 유용.
- 컨테이너 런타임이 비밀값의 값을 읽어 오는 곳이고 대상 위치는 컨테이너 안에서 비밀값이 위치할 경로를 의미
</aside>
...
1. environment : 컨테이너 안에서만 사용되는 환경 변수를 추가
2. env_file : 텍스트 파일의 경로를 값으로 받는다. 이 파일에 정의된 환경 변수가 컨테이너에 적용된다.
3. secrets : services나 networks 처럼 컴포즈 파일의 최상위 프로퍼티
</aside>
...
# $ 기호가 붙은 중괄호 부분이 호스트 컴퓨터의 환경 변수 값으로 치환된다.
```
...
- 비밀값에 설정값을 지정하는 방법은 모든 컨테이너 런타임에서 적용 가능하고 민감한 정보가 유출될 우려도 없기 때문에 유연성 면에서 가장 뛰어나다. 실제값이 어디에 저장되든 애플리케이션이 실행될 때 컨테이너 속의 특정한 파일로 전달된다.
</aside>
...
서비스 간 많은 설정값을 공유하는 컴포즈 파일의 덩치가 점점 커지는 문제가 발생
...
- 확장 필드는 YAML의 여러 블록을 한곳에서 정의하는 기능이다.
...
* CI는 정기적으로 반복되며 애플리케이션을 빌드하고 일련의 테스트를 수행하는 절차다.
#
